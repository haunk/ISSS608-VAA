...
# Flag variables that might need attention
outlier_summary %>%
  filter(
    pct_over_3sd > 5 |        # more than 5% of non-zero rows are extreme
    max_val > p999 * 100      # max is 100x higher than 99.9th percentile
  )
...
motoinsu %>%
  ...
  ggplot(aes(x = reorder(!!sym(group_var), loss_ratio), y = loss_ratio, fill = !!sym(group_var))) +
  geom_col() +
  geom_text(aes(label = paste0(round(loss_ratio, 1), "%")), hjust = -0.2) +
  coord_flip() +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  labs(title = paste("Loss Ratio by", group_var)) +
  theme_minimal()
...
-   COMP_N is the most unprofitable product at 93%, nearly breaking even on every euro collected - immediate repricing is needed. The 19-point gap between COMP_N (93%) and COMP_E (73.9%) demonstrates that the excess/deductible is highly effective at reducing claims cost.
-   TP is the most profitable at 54.6%, low claim frequency from limited coverage scope offsets its high individual claim severity.
-   TPG (69.6%) is surprisingly less profitable than CC (63.5%) despite simpler coverage, suggesting glass replacement costs are underpriced relative to their frequency.
-   Overall, three out of five products sit above the 60-70% healthy benchmark, consistent with the worsening loss ratio trend seen in the temporal analysis.
:::
...
::: callout-warning
-   Prioritize **RENAULT and PEUGEOT** for retention — high volume, low risk.
-   Review **OPEL and SEAT** pricing urgently — they generate the least revenue while being the most unprofitable.
:::
